...
const EditAvis = () => {
  ...
  useEffect(() => {
    ...
    avisService
      .getOneAvisAdmin(id)
      .then((response) => {
        const avis = response.data.data;
        console.log(avis);
        // définir les valeurs du formulaire d'edition
        Object.keys(avis).forEach((key) => {
          setValue(key, avis[key]);
        });
        setAvis(avis);
      })
      .catch((error) => {
  ...
  return (
    <>
      <div className="bg-gray-50 flex flex-row justify-between  p-5 mb-10 ">
        <p className="text-2xl text-[#9f0712] font-bold">
          Modifier l'avis avec id : {avis.id}
        </p>
      </div>
